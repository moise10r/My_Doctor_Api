...
router.get('/', [verifyToken], async(req, res) => {
  ...
  if(user) {
    ...
    return res.header('x-auth-token').status(400).send('This credential does not exist!');
  }
...
router.post('/:doctorId', [verifyToken], async(req, res) => {
  ...
  if(validate.error) {
    return res.header('x-auth-token').status(404).send(validate.error.details[0].message);
  }
  const user = await User.findById({ _id: req.user._id });
  if(!user) return res.header('x-auth-token').status(404).send('The user with this credential does not exist');
  const doctor = await Doctor.findById({ _id: req.params.doctorId });
  if(!doctor) return res.header('x-auth-token').status(404).send('The doctor with this Id does not exist');
  ...
  if(appointment) return res.header('x-auth-token').status(404).send('This appointement has been yet taken ');
...
router.get('/all', async(req, res) => {
  ...
  if(!appointements) return res.header('x-auth-token').send('No appointement taken!');
  return res.header('x-auth-token').status(200).send(appointements);
});

router.get('/doctors/:doctorId', async(req, res) => {
  const appointement = await Appointment.find({ "doctor._id": req.params.doctorId });
  if(!appointement) return res.header('x-auth-token').send('No appointement taken!');
